...
int keys_function(int keycode, void *param)
{
    ...
    if (keycode == XK_w && check != 0)
        data->player.p_y -= PLAYER_STEP;
    if (keycode == XK_d && check != 0)
        data->player.p_x += PLAYER_STEP;
    if (keycode == XK_s && check != 0)
        data->player.p_y += PLAYER_STEP;
    if (keycode == XK_a && check != 0)
        data->player.p_x -= PLAYER_STEP;
    if ((keycode == XK_w || keycode == XK_d || keycode == XK_s || keycode == XK_a) && check != 0)
    {
        data->moves_counter++;
